...
class Conv2DLayer(tf.keras.layers.Layer):
    ...
    A simple custom Conv layer containing Conv2D, a Normalizarion layer and an Activation at the end. 
    ...
    def __init__(self, filters: int, kernel_size: tuple[int, int], 
        ...
        if not isinstance(kernel_size, tuple) or len(kernel_size) != 2 or not all(isinstance(k, int) and k > 0 for k in kernel_size):
            raise ValueError(f"kernel_size must be a tuple of two positive integers. Received: {kernel_size} with type {type(kernel_size)}")
        if not isinstance(strides, tuple) or len(strides) != 2 or not all(isinstance(s, int) and s > 0 for s in strides):
            raise ValueError(f"strides must be a tuple of two positive integers. Received: {strides} with type {type(strides)}")
        ...
        self.convolution = tf.keras.layers.Conv2D(filters=self.filters, kernel_size=self.kernel_size, 
    ...
    def compute_output_shape(self, input_shape: Union[tf.TensorShape, tuple[int, int, int, int]]) -> tuple[int, int, int, int]:
        ...
        -------
        output_shape : tuple
            Shape of the output tensor after applying the Conv2D layer. 
        
        """


        return self.convolution.compute_output_shape(input_shape)
