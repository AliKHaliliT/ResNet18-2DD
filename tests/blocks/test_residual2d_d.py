import unittest
from ResNet182DD.assets.blocks.residual2d_d import Residual2DD
import tensorflow as tf
import inspect


class TestResidual2DD(unittest.TestCase):

    def test_creation_init_layer(self):

        # Arrange and Act
        layer = Residual2DD(filters=1, strides=(1, 1))
    ...
    def test_output_tensor_intended__shape(self):
        ...
        # Act
        output = Residual2DD(filters=filters, strides=strides)(input_tensor)
    ...
    def test_get__config_init_matching__dict(self):
        ...
        init_params = [
            param.name
            for param in inspect.signature(Residual2DD.__init__).parameters.values()
